...
bool compareAngles(const Point& a, const Point& b) {
    ...
    if (convex(*origin, b, a))
        ...
        double distA = abs(a.x - origin->x) + abs(a.y - origin->y);
        double distB = abs(b.x - origin->x) + abs(b.y - origin->y);
        return distA < distB;
    }
}
...
int main(int argc, char** argv) {
    ...
    drawLine(*pixMap, *convex_points[0], *convex_points[convex_points.size() - 1], Color (true,
    false, true)); drawPoints(*pixMap, *points, convex_points); generateImage(*pixMap);
    */
    return 0;
}